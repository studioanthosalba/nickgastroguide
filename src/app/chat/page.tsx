  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ 
          messages: [
            { role: 'system', content: "Sei Nick, l'AI Concierge e Sommelier digitale esclusivo di Nick GastroGuide. Sii raffinato e utile." },
            ...newMessages
          ],
          restaurantSlug: 'general' // Default context if needed
        })
      });
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: "Mi scuso, il mio collegamento in cantina è debole al momento. Riprova tra poco." }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="font-body text-on-surface overflow-x-hidden min-h-screen bg-[#131313] flex flex-col">
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#131313]/80 backdrop-blur-xl flex justify-between items-center px-6 py-4 shadow-[0_20px_40px_rgba(229,226,225,0.04)]">
        <div className="flex items-center gap-3">
          <Link href="/dashboard" className="text-tertiary-container hover:text-primary transition-colors mr-2">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div className="w-10 h-10 rounded-full overflow-hidden border border-outline-variant/30">
            <img 
              alt="Nick the Sommelier" 
              className="w-full h-full object-cover" 
              src="https://lh3.googleusercontent.com/aida-public/AB6AXuDA3augtRTPQhRt_jX4gzAvNWqLDp8B-pghi-USNQcfm-ccACycdw1eCiG5lX5pbcK1d-gi3ycXisI0vVMJn8Ar9cJOHUcVoUJ2ALOzEKSMO_el9egL5QbEWzRdgXKnXTexAzeqlo9q-GaDCDnK1lu8OALySHAHn087lsmvl_ISDFer94m8txOxlf5kQoZPAwhFZ94DnCs-rlO4Ts_8C6Z1kl4_RxuRIscP2hvjcpaBLQQ4abLEHfGBfrsSj7A2YbSKXV5H6ASegjgY" 
            />
          </div>
          <span className="text-xl font-headline italic text-primary tracking-tight">Nick GastroGuide</span>
        </div>
        <button className="text-primary hover:opacity-80 transition-opacity active:duration-150 scale-95">
          <span className="material-symbols-outlined select-none" style={{ fontVariationSettings: "'FILL' 0" }}>notifications</span>
        </button>
      </nav>
        ...
        >
          <div className="w-20 h-20 rounded-lg overflow-hidden flex-shrink-0">
            <img 
              alt="Nick Profile" 
              className="w-full h-full object-cover" 
              src="https://lh3.googleusercontent.com/aida-public/AB6AXuBJCO2NgDzKtFrFwxn-82qjdAsLz8wQjH1vMM1fIzVSBcVqjOKpytr_hbF4c9uEZ499qYg1xDdBZC0dM3kbhy55XY4z8sND_8_jtMd14-YmevxPdSWGKKTfIPBZxn_DB2mcwjd5QTLjJyBh-q783AzI7yzlQPkDWkWdpbmFP_uUeUtdwHaGcOEXsFfPSdJLyzaCWTjCk8Vz69vKt4hJGkxb5k32bqzadWDvp5VrTtMIClYXgdGlTMh5XSCPdtsbMbNUyLkKc0vBUj40" 
            />
          </div>
          <div className="flex flex-col">
            <span className="font-label font-medium uppercase tracking-widest text-[10px] text-primary mb-1">YOUR AI CONCIERGE</span>
            <h1 className="text-2xl font-headline italic leading-tight text-on-surface mb-2">Benvenuto sono Nick</h1>
            <p className="text-[11px] font-label uppercase tracking-widest text-tertiary-container">Curated by Nick, Michelin * 2024</p>
          </div>
        </motion.header>
          ...
          <AnimatePresence>
            {messages.map((m, idx) => (
              m.role === 'user' ? (
                <motion.div key={idx} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-end gap-1">
                  <span className="text-[9px] font-label uppercase tracking-widest text-tertiary-container mr-2">YOU</span>
                ...
                <motion.div key={idx} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-start gap-1">
                  <span className="text-[9px] font-label uppercase tracking-widest text-tertiary-container ml-2">NICK</span>
            ...
            {isLoading && (
              <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="flex items-center gap-3 pl-2 mt-4">
                ...
                <span className="text-[9px] font-label uppercase tracking-widest text-tertiary-container">NICK IS CURATING...</span>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="fixed bottom-0 left-0 w-full z-50">
        <div className="px-4 py-4 bg-[#131313]/95 backdrop-blur-2xl border-t border-outline-variant/10">
          ...
          <div className="flex gap-2 overflow-x-auto hide-scrollbar mb-4 pb-1 max-w-2xl mx-auto">
            {['NEI DINTORNI?', 'COSA MI CONSIGLI?', 'MENU', 'CANTINA'].map((pill) => (
              <button 
                key={pill} 
